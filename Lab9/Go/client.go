...
// Функция для изменения имени или email
func updateUserInfo() {
	if !isAuthenticated() {
		fmt.Println("Необходимо авторизоваться!")
		return
	...
	case 1:
		fmt.Print("Введите новое имя: ")
		fmt.Scan(&user.Name)
		// Отправка только имени на сервер
		data, _ := json.Marshal(user)
		resp := makeRequest("PUT", "http://localhost:8000/users/update-info", data)
		defer resp.Body.Close()

		body, _ := io.ReadAll(resp.Body)
		if resp.StatusCode == http.StatusOK {
			fmt.Println("Имя пользователя обновлено:", string(body))
		} else {
			fmt.Println("Ошибка обновления имени пользователя:", string(body))
		}
	case 2:
		fmt.Print("Введите новый email: ")
		fmt.Scan(&user.Email)
		// Отправка только email на сервер
		data, _ := json.Marshal(user)
		resp := makeRequest("PUT", "http://localhost:8000/users/update-info", data)
		defer resp.Body.Close()
		...
		body, _ := io.ReadAll(resp.Body)
		if resp.StatusCode == http.StatusOK {
			fmt.Println("Email пользователя обновлен:", string(body))
		} else {
			fmt.Println("Ошибка обновления email пользователя:", string(body))
		}
	default:
...
func main() {
	for {
		...
		fmt.Println("7. Обновить имя или email") // Добавлен новый пункт меню
		fmt.Println("8. Выход из системы")
		fmt.Println("0. Выход")
		...
		case 6:
			deleteUser()
		case 7:
			updateUserInfo() // Новый пункт меню
		case 8:
			logout()
		case 0:
